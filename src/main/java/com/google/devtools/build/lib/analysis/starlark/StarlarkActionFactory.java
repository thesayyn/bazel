...
import com.google.devtools.build.lib.analysis.actions.LinkAction;
...
public class StarlarkActionFactory implements StarlarkActionFactoryApi {
  ...
  public void link(
      ...
      StarlarkThread thread)
      throws EvalException {
    context.checkMutable("actions.link");
    RuleContext ruleContext = getRuleContext();
    ...
    if (outputArtifact.isSymlink()) {
      throw Starlark.errorf(
          "link() requires that \"output\" be declared as a file or directory, not a symlink (did"
    ...
    if (inputArtifact.isDirectory() != outputArtifact.isDirectory()) {
      ...
      String outputType = outputArtifact.isDirectory() ? "directory" : "file";
      throw Starlark.errorf(
          "link() requires that \"output\" (%s) and \"target_file\" (%s) be the same type",
